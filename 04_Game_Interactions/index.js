...
canvas.addEventListener("pointerdown", initDrag);
canvas.addEventListener("pointermove", dragging);
canvas.addEventListener("pointerup", stopDragging);
canvas.addEventListener("pointerleave", stopDragging);

function initDrag({offsetX, offsetY}) {
  let trashEl = trash[0];

  trashEl.isDragged(offsetX, offsetY);
}

function dragging({offsetX, offsetY}) {
  let trashEl = trash[0];
